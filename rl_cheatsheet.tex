...
\textbf{Softmax policy}: \smash{$\pi_\theta(s,a)=e^{\phi(s,a)^\top\theta}/\left(\sum\limits_a e^{\phi(s,a)^\top\theta}\right)$}. 

\textit{Score function}: $\nabla_\theta\log\pi_\theta(s,a)=\phi(s,a)-\mathbb{E}_{\pi_\pi}[\phi(s,a)].
$

\textbf{Gaussian policy}: \smash{$\pi_\theta(a(s)\sim\mathcal{N}(\mu_\theta(s), \sigma^2)$}. 
